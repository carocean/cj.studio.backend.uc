...
public interface IAccountStub {
	...
	void removeAccount(@CjStubInParameter(key = "accountCode", usage = "账户") String accountCode);

	@CjStubMethod(usage = "获取账户")
	@CjStubReturn(type = Account.class, usage = "返回账户，如果不存在则返回null")
	Account getAccount(@CjStubInParameter(key = "accountCode", usage = "账户") String accountCode);
	...
	void removeSegmentOfAccount(@CjStubInParameter(key = "accountCode", usage = "账户") String accountCode,
			@CjStubInParameter(key = "segCode", usage = "段号") String segCode);

	@CjStubMethod(usage = "清空账户的信息段")
	void emptySegmentsOfAccount(@CjStubInParameter(key = "accountCode", usage = "账户") String accountCode);
	...
	void removeAccountAttribute(@CjStubInParameter(key = "accountCode", usage = "账户") String accountCode,
			@CjStubInParameter(key = "segCode", usage = "段号") String segCode,
			@CjStubInParameter(key = "attrCode", usage = "属性") String attrCode);

	@CjStubMethod(usage = "清空账户指定段下的所有属性")
	void emptyAccountAttributes(@CjStubInParameter(key = "accountCode", usage = "账户") String accountCode,
			@CjStubInParameter(key = "segCode", usage = "段号") String segCode);
